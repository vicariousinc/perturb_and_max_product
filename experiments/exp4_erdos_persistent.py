...
# Erdös-Renyi graph with persistent training
...
def run(learn_iter, n_steps, sampling_alg, seed, n_samples=256, signed=True):
    ...
    if sampling_alg == "pmap":
        ...
        n_samples, n_steps = int(np.floor(10 * np.sqrt(n_steps))), 40
...
if __name__ == "__main__":
    ...
    for n_steps in [5, 10, 25, 50, 100]:
        ...
        for sampling_alg in ["pmap", "gibbs", "gwg"]:
            ...
            np.savez_compressed(
                f"{folder}/erdos_li_{learn_iter}_{sampling_alg}_nsteps_{n_steps}_seed_{seed}.npz",
                logmmd=logmmd,
                logrmse=logrmse,
                Wgt=Wgt,
                bgt=bgt,
